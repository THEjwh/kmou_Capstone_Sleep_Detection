...
##params = {'alpha_list':np.arange(0.01,10,0.01)}
##grid = GridSearchCV(ridge,params,n_jobs =-1)
##grid.fit(train_input,train_target)
##print(grid.best_params_)  #최상 조합 출력
##print(np.max(grid.cv_results_['mean_test_score']))
##
...
##grid_L = GridSearchCV(lasso,params,n_jobs =-1)
##grid_L.fit(train_input,train_target)
##print(grid_L.best_params_)  #최상 조합 출력
##print(np.max(grid_L.cv_results_['mean_test_score']))
##
###최적의 모델은 best_estimator_ 속에 저장되어 있음
##dt2=grid_L.best_estimator_
